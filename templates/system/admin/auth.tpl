    ...
    <div class="form-box">
        <form method="POST" id="auth" action="{adminUrl mod_controller=false do=false Act="auth"}" class="body-box" {if $data.do == 'recover'} style="display:none"{/if}>
            ...
            <div class="field-zone">
                <input type="text" class="login" name="login" placeholder="{t}Логин{/t}" value="{$data.login|default:$Setup.DEFAULT_DEMO_LOGIN}">
        ...
        <form method="POST" id="recover" action="{adminUrl mod_controller=false do="recover" Act="auth"}" class="body-box recover" {if $data.do != 'recover'}style="display:none"{/if}>
            <div class="field-zone">
                <input type="text" id="login" class="login" name="login" placeholder="E-mail" value="{$data.login|escape}">
                <p class="help">
                    <i class="corner"></i>
                    {t}На указанный E-mail будет отправлено письмо с дальнейшими инструкциями по восстановленю пароля{/t}
                </p>
            </div>
            <p class="buttons">
