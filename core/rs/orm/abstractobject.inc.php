...
abstract class AbstractObject extends AcceptBehavior implements \ArrayAccess, \Iterator
{
    ...
    protected $_values = array(); //Значения свойств
    ...
    protected static $default_local_parameters = array(
    ...
    protected static $default_class_parameters = array(
    ...
    */
    ...
    public function insert($ignore = false, $on_duplicate_update_keys = array(), $on_duplicate_uniq_fields = array())
    {
         ...
         */
        $can_do = $this->beforeWrite(self::INSERT_FLAG, $on_duplicate_update_keys) !== false && !EventManager::fire('orm.beforewrite.' . $this->getShortAlias(), array(
                    'orm' => $this,
        ...
        if ($can_do && $ret = $storage->insert($type, $on_duplicate_update_keys, $on_duplicate_uniq_fields)) {
             ...
             * index 'flag' => string "insert" | "replace" | "update"
             */
            EventManager::fire('orm.afterwrite.' . $this->getShortAlias(), array(
                'orm' => $this,
                'flag' => self::INSERT_FLAG,
                'on_duplicate_update_keys' => $on_duplicate_update_keys
            ));
    ...
    public function update($primaryKey = null)
    {
         ...
         * index 'flag' => string "insert" | "replace" | "update"
         */
        $can_do = $this->beforeWrite(self::UPDATE_FLAG) !== false && !EventManager::fire('orm.beforewrite.' . $this->getShortAlias(), array(
                    'orm' => $this,
    ...
    public function replace()
    {
         ...
         * index 'flag' => string "insert" | "replace" | "update"
         */
        $can_do = $this->beforeWrite(self::REPLACE_FLAG) !== false && !EventManager::fire('orm.beforewrite.' . $this->getShortAlias(), array(
                    'orm' => $this,
    ...
    public function delete()
    {
        ...
        $eresult = EventManager::fire('orm.delete.' . $this->getShortAlias(), array(
            'orm' => $this,
        ));
     ...
     */
    public function getFromArray(array $data,
                                 $remove_prefix = null,
                                 $mark_modify = true,
                                 $call_afterload_event = false)
    {
        ...
        if ($call_afterload_event) {
            ...
            EventManager::fire('orm.afterload.' . $this->getShortAlias(), array(
                'orm' => $this,
            ));
        }
    ...
    protected static function _tableName($with_quotes = true)
    {
        if (!isset(static::$table)) {
            throw new RSException(t('Не задано имя таблицы в ORM объекте %0', get_called_class()));
        }
    ...
    {
        return "`" . static::$db . "`." . static::_tableName();
    }
