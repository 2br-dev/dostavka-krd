...
class EntityList extends BaseModel
{
    ...
    protected $assocPlainList = array(); //Полностью загруженный список
    protected $parsePrefixes = array('&' => 'AND', '|' => 'OR');
    /** @var \RS\Orm\Request */
    protected $q;

    function __construct(AbstractObject $orm_element, array $options = array())
    {
        $this
    ...
    function getById($id)
    {
        ...
        if ($this->alias_field) {
            $q->where("`{$this->alias_field}` = '#alias_or_id'", array('alias_or_id' => $id), 'OR')
                ->orderby("{$this->alias_field} = '#alias_or_id' desc", array('alias_or_id' => $id));
        }
        $q->closeWGroup();
     ...
     * @return array
     */
    function getSelectList($first = array())
    {
        $result = array();
        $list = $this->getList();
        foreach ($list as $v) {
    ...
    function saveRequest($key)
    {
        if (!isset($_SESSION[self::WHERE_CONDITION_VAR]) || !is_array($_SESSION[self::WHERE_CONDITION_VAR])) {
            $_SESSION[self::WHERE_CONDITION_VAR] = array();
        }
