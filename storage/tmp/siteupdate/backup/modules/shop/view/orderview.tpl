    ...
    <div class="order-columns" style="margin-bottom:20px">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {if $links}
                                            ...
                                            {/foreach}
                                        {else}
                                            Нет. <a href="{$router->getAdminUrl('CreateFromOrder', ['order_id' => $elem.id], 'catalog-inventoryreservationctrl')}" class="crud-get createReserve">Создать резервирование</a>
                                        {/if}
                                    </td>
                                </tr>
                            {/if}
        ...
        <div class="o-rightcol">
            ...
            <div id="paymentBlockWrapper" class="bordered">
                {$pay = $elem->getPayment()}
                {hook name="shop-orderview:payment" title=t('Редактирование заказа(админ. панель):Блок оплаты') pay=$pay elem=$elem order_consistent=true}
