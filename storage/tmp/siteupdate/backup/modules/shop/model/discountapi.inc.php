...
class DiscountApi extends EntityList
{
    ...
    public static function applyCouponPercentDiscountsToCart(Cart $cart)
    {
        ...
        foreach ($coupon_items as $key => $coupon_item) {
            ...
            if ($coupon['discount_type'] == Discount::DISCOUNT_TYPE_PERCENT) {
                ...
                if ($min_order_price > $cart_total || $date_expire || !$coupon['active']) {
                    $cart->removeItem($key);
                } else {
     ...
     * @throws RSException
     */
    public function save($id = null, array $user_post = array()): bool
    {
     ...
     * @throws DbException
     */
    public function multiUpdate(array $data, $ids = array())
    {
