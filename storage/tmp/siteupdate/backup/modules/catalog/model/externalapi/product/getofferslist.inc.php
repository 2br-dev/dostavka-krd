...
class GetOffersList extends \ExternalApi\Model\AbstractMethods\AbstractGet
{
    ...
    */
    protected function getProductOfferType($product)
    {
        ...
        if ($product->isMultiOffersUse() && $product->isOffersUse()){ //Если есть многомерные комплектации и комплектации
    ...
    */
    protected function getProductOffers($product)
    {
        ...
        $offer->getPropertyIterator()->append(array(
                    'num' => new \RS\Orm\Type\Integer(array(
                        'description' => t('Остаток на складе'),
                        'visible' => true,
                    )),
                    'sortn' => new \RS\Orm\Type\Integer(array(
                        'description' => t('Сортировочный индекс'),
                        'visible' => true,
                    )),
                    'propsdata_arr' => new \RS\Orm\Type\ArrayList(array(
                        'description' => t('Характеристики комплектации'),
                        'visible' => true,
                    )),
                    '_propsdata' => new \RS\Orm\Type\ArrayList(array(
                        'description' => t('Характеристики комплектации'),
                        'visible' => false,
                    )),
                    'cost_values' => new \RS\Orm\Type\ArrayList(array(
                        'description' => t('Розничная и зачёркнутая цена товара'),
                        'appVisible' => true,
                    )),
                    'all_cost_values' => new \RS\Orm\Type\ArrayList(array(
                        'description' => t('Все цены'),
                        'appVisible' => true,
                    )),
                    'button_type' => new \RS\Orm\Type\Integer(array(
                        'description' => t('Тип кнопки в зависимости от комплектации, наличия и т.д.'),
                        'visible' => true,
                    )),
                    'stock_sticks' => new \RS\Orm\Type\MixedType(array(
    ...
    */
    ...
    {
        $value->getPropertyIterator()->append(array(
            'images' => new \RS\Orm\Type\ArrayList(array(
        ...
        if (!empty($multioffers)){
            ...
            */
            foreach($multioffers as $multioffer_level){
                ...
                $multioffer_level->getPropertyIterator()->append(array(
                    'values' => new \RS\Orm\Type\Integer(array(
                        'description' => t('Значения уровня многомерных комплектаций'),
                        'visible' => true,
                        'Appvisible' => true,
                    )),
                    'property_type' => new \RS\Orm\Type\Varchar(array(
                    ...
                    */
                    foreach ($multioffer_level['values'] as $key=>$value){
                        switch($multioffer_level['property_type']){ //Укажем значения для типов характеристики цвет и изображение
                            case 'color':
                                    $value->getPropertyIterator()->append(array(
                                        'color' => new \RS\Orm\Type\Varchar(array(
                                            'description' => t('Цвет'),
                                            'visible' => true,
                                            'Appvisible' => true,
                                        )),
    ...
    */
     ...
     */
    function process($token = null, $product_id, $sections = array('stock', 'costs'))
    {
        //Загруженный товар
        ...
        if ($product['id']){
            ...
            $response['response']['offers']     = array();
        }
