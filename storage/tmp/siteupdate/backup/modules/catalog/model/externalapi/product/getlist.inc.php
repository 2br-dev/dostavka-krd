...
class GetList extends \ExternalApi\Model\AbstractMethods\AbstractGetList
{
    ...
    protected
        ...
        $dirs_x_id = array(), //Массив категорий с ключами id
        $list_products,
    ...
    function getDaoObject()
    {
        if ($this->dao == null){
            ...
            $this->dao->getElement()->getPropertyIterator()->append(array(
                'num' => new \RS\Orm\Type\Decimal(array(
                    'maxLength' => 11,
                    'decimal' => 3,
                    'allowEmpty' => false,
                    'appVisible' => true
    ...
    {
        return array('id', 'id desc', 'dateof', 'dateof desc', 'dateof asc',
    ...
    protected function addDirData($dao)
    {
        ...
        $product->getPropertyIterator()->append(array(
            'specdirs' => new \RS\Orm\Type\ArrayList(array(
                'description' => t('Спец. категории'),
                'appVisible' => true
            )),
            'category' => new \RS\Orm\Type\MixedType(array(
        ...
        if (!empty($this->list_products)){
            foreach ($this->list_products as $product){
                ...
                $product['category'] = $category ? $category : array('name' => t('Bce'));
            }
        }
    }
    ...
    protected function addImageData($dao)
    {
        ...
        if (in_array('image', $this->method_params['sections'])) {
            $dao->getElement()->getPropertyIterator()->append(array(
                'image' => new \RS\Orm\Type\ArrayList(array(
    ...
    protected function addConcomitantData($dao)
    {
        if (in_array('concomitant', $this->method_params['sections'])) {
            $dao->getElement()->getPropertyIterator()->append(array(
                'concomitant' => new \RS\Orm\Type\ArrayList(array(
    ...
    protected function addPropertyData($dao)
    {
        if (in_array('property', $this->method_params['sections'])) {
            $dao->getElement()->getPropertyIterator()->append(array(
                'property_values' => new \RS\Orm\Type\ArrayList(array(
    ...
    function process($token = null,
                     ...
                     $page = 1,
                     $pageSize = 20,
                     $sections = array('image', 'cost', 'property', 'concomitant', 'unit', 'current_currency', 'filters'))
    {
        ...
        if (!empty($this->list_products)){ //Если товары найдены
            ...
            if (in_array('unit', $sections)) {
                ...
                $unit_api->setFilter('id', array_merge(array_keys($units), array($default_unit)), 'in');
