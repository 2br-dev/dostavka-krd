    ...
    */
    protected function process($token, $client_id, $client_secret, $product_id, $offer_id, $values)
    {
        ...
        if ($product->isOffersUse() && $offer_id){ //Если есть комплектации
            ...
            if (!$offer){
                throw new ApiException(t('Комплектация товара с артикулом %0 не найдена.', array($offer_id)));
            }
        }else{
        ...
        foreach($values as $warehouse_id=>&$value){
            if (!is_numeric($warehouse_id)){
                ...
                if (!$warehouse){
                    throw new ApiException(t('Склад %0 - не найден. Проверте название.', array($warehouse_id)));
                }
                ...
                if (!$warehouse['id']){
                    throw new ApiException(t('Склад не найден.', array($warehouse_id)));
                }
            }
