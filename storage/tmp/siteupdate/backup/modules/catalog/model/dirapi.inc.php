...
class DirApi extends TreeCookieList
{
    ...
    protected $max_level = 20;

    public function __construct()
    {
        parent::__construct(new Dir(), array(
            'multisite' => true,
            'parentField' => 'parent',
            'sortField' => 'sortn',
            'idField' => 'id',
    ...
    function multiUpdate(array $data, $ids = array())
    {
        ...
        $event_result = EventManager::fire('orm.beforemultiupdate.catalog-dir',array(
            'data' => $data, 
            'ids' => $ids, 
            'api' => $this, 
        ));
        ...
        EventManager::fire('orm.multiupdate.catalog-dir',array(
            'ids' => $ids,
            'api' => $this, 
        ));
        return $ret;
    ...
    public function listWithAll()
    {
        ...
        foreach($treedata as $item) {
            ...
            if ($dir['is_spec_dir'] == 'Y') {
                $dir['treeTools'] = new \RS\Html\Table\Type\Actions('id', array(
    ...
    {
        $first = ($include_root) ? array(0 => t('Верхний уровень')) : array();
     ...
     * @throws \RS\Exception
     */
    public static function staticSpecTreeList($first_elements = array())
    {
        $_this = new self();
        $_this->setFilter('is_spec_dir', 'Y');
     ...
     * @throws \RS\Exception
     */
    public static function staticNoSpecTreeList($first_elements = array())
    {
        $_this = new self();
    ...
    function save($id = null, array $user_post = array())
    {       
        if ($id !== 0) {
    ...
    */
    function addTranslitAliases($count_def = 0)
    {
        ...
        while($row = $res->fetchRow()) {
            ...
            $ok = false;
            while(!$ok && $i<15) {
                $dir[$this->alias_field] = Transliteration::str2url($dir['name']).(($i>0) ? "-$i" : '');
                $ok = $dir->update();
                $i++;
            }
        }
