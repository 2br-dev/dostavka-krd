...
class PropertyApi extends EntityList
{
    ...
    public $last_filtered_props = array();
    ...
    */
    ...
    function getGroupProperty($group_id, $include_parent = true, $public = null)
    {
        ...
        $hash = implode(",", array( //Хеш для ключа кэша
            $group_id,
            $include_parent ? 1 : 0,
            $public ? 1 : 0,
        ));
        if ($this->use_static_cache && !isset($cache[$hash])) {
            ...
            $res = OrmRequest::make()
                ...
                ->where($public_where)
                ->orderby("field(group_id, $group_list_sql)")
            ...
            foreach ($res as $prop_id => $prop_links) {
                ...
                $item['value'] = ($types_data[$item['type']]['is_list']) ? array() : null ;
     ...
     * @throws \RS\Exception
     */
    function checkPropertyFiltersExitsInCategoryDir($dir_id, $brands_in_dir, $prop_filters = array(), $base_filters = array())
    {
        $config = \RS\Config\Loader::byModule($this);
        $flag = true; //Флаг, что всё в порядке
        $prop_filters  = array_keys($prop_filters);
        $brands        = array_keys($brands_in_dir);
        $brand_filters = isset($base_filters['brand']) ? $base_filters['brand'] : array();
    ...
    */
     ...
     * @param bool $allowed_filters - разрешенные фильты
    ...
    function getSwitchedOffFilters($filters, $dir_id = null, $base_filters = false, $bfilters_allowed = null)
    {
        ...
        $allowed_filters_1 = array(); // массив свойств фильтров
        $prop_ids = array(); // массив id характеристик
        ...
        return array('allowed_filters' => $allowed_filters_1, 'filters' => $filters, 'bfilters' => $base_filters);
    }
}
