...
class ImportYmlApi extends BaseModel
{
    ...
    protected $timeout;
    protected $config;
    protected $allow_ext = array('yml', 'xml');
    ...
    public $storage;
    public $xmlIds;
    public $cachecheck = array();

    public function __construct()
    {
        $this->yml_folder = \Setup::$PATH . $this->yml_folder_rel;
        $this->log_file = RSLog::file(\Setup::$PATH . \Setup::$STORAGE_DIR . self::LOG_FILE);
        $this->log_file->enableDate()->setMaxLength(10485760); // 10 МБ
    ...
    public function getSteps($step_data)
    {
        ...
        if (in_array($this->config['catalog_element_action'], array(CatalogConfig::ACTION_REMOVE, CatalogConfig::ACTION_DEACTIVATE, CatalogConfig::ACTION_CLEAR_STOCKS))) {
            $after_import_products_step = array(
            ...
            $steps = array_merge($steps, array($after_import_products_step));
        }

        if (in_array($this->config['catalog_section_action'], array(CatalogConfig::ACTION_REMOVE, CatalogConfig::ACTION_DEACTIVATE))) {
            $after_import_dirs_step = array(
            ...
            $steps = array_merge($steps, array($after_import_dirs_step));
        }
    ...
    private function stepStart($step_data)
    {
        ...
        $this->writeLog(t('Начало импорта'));
    ...
    {
        $this->writeLog(t("Шаг импорта валют - начало"));
        ...
        if ($reader->readOuterXml()) {
            ...
            foreach ($currencies as $curr_xml) {
                ...
                if (!$currency['id']) {
                    $this->writeLog(t("Создание валюты") . "\"{$currency_data['title']}\"");
        ...
        $this->writeLog(t("Шаг импорта валют - конец"));
        return true;
    }
    ...
    {
        $this->writeLog(t("Шаг импорта категорий - начало"));
        //id категорий
        ...
        do {
            ...
            if (!isset($dir['id'])) {
                $this->writeLog(t("Создание категории") . "\"{$dirs[$offset]['name']}\"");
                ...
                $this->writeLog(t("Обновление категории") . "\"{$dirs[$offset]['name']}\"");
                $dir['processed'] = 1;
                $dir->update();
        ...
        Dirapi::updateLevels();
        $this->writeLog(t("Шаг импорта категорий - конец"));
        return true;
    }
    ...
    {
        $this->writeLog(t("Шаг импорта товаров - начало"));
        ...
        $this->writeLog(t("Шаг импорта товаров - конец"));
        return true;
    }
    ...
    {
        $this->writeLog(t("Шаг импорта рекомендованных - начало"));
        if ($step_data['offset'] == 0) {
        ...
        $this->writeLog(t("Шаг импорта рекомендованных - конец"));
        return true;
    }
    ...
    {
        $this->writeLog(t("Шаг импорта фотографий - начало"));
        $reader = $this->loadReader('offer');
                ...
                foreach ($offer_xml->picture as $picture) {
                    ...
                    if (!$image) {
                        if ($photoapi->addFromUrl($picture, 'catalog', $product_id, true, $xml_id, false)) {
                            ...
                            $this->writeLog(t('Загружено фото') . "\"$picture\"");
                        } else {
                            $this->storage['statistic']['not_downloaded_photos']++;
                            $this->writeLog(t('Не удалось загрузить фото') . "\"$picture\"");
        ...
        $this->writeLog(t("Шаг импорта фотографий - конец"));
        return true;
    }
    ...
    private function importProduct($reader)
    {
        ...
        $this->writeLog(t('Импорт товара') . " $xml_id \"$title\"");
        ...
        if (isset($offer_xml->price)) {
            ...
            $excost_array[$this->getCostId()] = array(
                'cost_original_val' => $this->config['increase_cost'] ? (string)$offer_xml->price * (1 + ($this->config['increase_cost'] / 100)) : (string)$offer_xml->price,
                'cost_original_currency' => $coc
            );
            if ($offer_xml->oldprice && $this->getOldCostId()) {
                $excost_array[$this->getOldCostId()] = array(
                    'cost_original_val' => $this->config['increase_cost'] ? (string)$offer_xml->oldprice * (1 + ($this->config['increase_cost'] / 100)) : (string)$offer_xml->oldprice,
                    'cost_original_currency' => $coc
                );
            }
        ...
        if ((string)$offer_xml->vendor) {
            ...
            if (!isset($this->storage['list']['brands'][$vendor_title])) {
                $this->writeLog(t("Создание бренда \"$vendor_title\""));
        ...
        if($this->config->save_product_public) {
            $product_new = Product::loadByWhere(array('xml_id' => $xml_id, 'site_id' => $this->site_id));
            if($product_new['id']){
        ...
        $product['alias'] = Transliteration::str2url($product['title'], true, 140) . "-" . $uniq_postfix;

        EventManager::fire('importyml.product.after', array(
            'product' => $product,
            'offer_xml' => $offer_xml,
            'api' => $this,
        ));

        $on_duplicate_update_fields = array('xml_id', 'title', 'description', 'public', 'maindir', 'barcode', 'processed', 'brand_id');
    ...
    private function updateProductParams(\SimpleXMLElement $offer_xml, $product_id)
    {
        ...
        foreach ($offer_xml->param as $param) {
            ...
            if ($checkproperty = Orm\Property\Item::loadByWhere(array(
                'xml_id' => $xml_id,
                'site_id' => SiteManager::getSiteId()))
            ) {
                if ($checkproperty->type == 'list') {
                    if ($itemvalue = Orm\Property\ItemValue::loadByWhere(array(
    ...
    {
        $this->writeLog(t('Создание характеристики') . " \"$title\" ($type)");
     ...
     * @return string
     */
    public static function getLogLink()
    {
        return \Setup::$STORAGE_DIR . self::LOG_FILE;
    }
