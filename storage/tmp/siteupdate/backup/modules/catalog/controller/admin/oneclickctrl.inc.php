    ...
    function helperEdit()
    {
        $helper = parent::helperEdit();
            ...
            if (\RS\Module\Manager::staticModuleExists('shop')) {
                $buttons['create'] = new ToolbarButton\Button($this->router->getAdminUrl('createorderfromoneclick', array('oneclick_id' => $oneclick['id'])), t('создать заказ'));
            }
    ...
    */
    function actionCreateOrderFromOneClick()
    {
        ...
        */
        ...
        Application::getInstance()->redirect($this->router->getAdminUrl('edit', array('id'=>$order['id']), 'shop-orderctrl'));
    }
}
