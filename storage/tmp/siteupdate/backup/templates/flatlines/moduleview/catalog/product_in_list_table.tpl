{* Список товаров в табличном виде *}

<div class="catalog-list_card-product">
    <div {$product->getDebugAttributes()} data-id="{$product.id}" class="rs-product-item card card-product {if count($imagelist)>1}rs-photo-view{/if}
        ...
        <div class="card-image">
            <a href="{$product->getUrl()}"><img src="{$product->getMainImage()->getUrl(476, 280)}" alt="{$product.title}"></a>
        </div>
        <div class="card-text">
                    ...
                    <div class="pull-right">
                        <div class="comments">
                            <a href="{$product->getUrl()}#comments">
                                ...
                                <span>{$product->getCommentsNum()}</span></a>
                        </div>
                    </div>
                </div>
            {/if}

            {if $list_properties=$product->getListProperties($category)}
        ...
        <div class="card-price">
            ...
            {/if}
            <span class="card-price_new">{$product->getCost()} {$product->getCurrency()}</span>
            <div>
                    ...
                    {if $shop_config && !$product.disallow_manually_add_to_cart}
                        {if $product->shouldReserve()}
                            {if $product->isOffersUse() || $product->isMultiOffersUse()}
                                <a data-url="{$router->getUrl('shop-front-multioffers', ["product_id" => $product.id])}" class="link link-more rs-in-dialog">{t}Заказать{/t}</a>
                            {else}
                                <a data-url="{$router->getUrl('shop-front-reservation', ["product_id" => $product.id])}" class="link link-more rs-in-dialog">{t}Заказать{/t}</a>
                            {/if}
                        {else}
                            ...
                            {else}
                                {if $product->isOffersUse() || $product->isMultiOffersUse()}
                                    <a data-url="{$router->getUrl('shop-front-multioffers', ["product_id" => $product.id])}" class="link link-more rs-in-dialog">{t}В корзину{/t}</a>
                                {else}
                                    <a data-url="{$router->getUrl('shop-front-cartpage', ["add" => $product.id])}" class="link link-more rs-to-cart rs-no-show-cart" data-add-text="{t}Добавлено{/t}">{t}В корзину{/t}</a>
                                {/if}
                            {/if}
                        {/if}
                    {/if}
