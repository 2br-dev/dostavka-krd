...
<div class="filter filter-clickable filter-color rs-type-multiselect {if $filters[$prop.id] || $prop.is_expanded}open{/if}">
    <a class="expand">
        <span class="right-arrow"><i class="pe-2x pe-7s-angle-down-circle"></i></span>
        <span>{t}{$prop.title}{/t} <span class="filter-remove rs-remove hidden" title="{t}Сбросить выбранные параметры{/t}"><i class="pe-va pe-7s-close-circle"></i></span></span>
    </a>
    <div class="detail">
        <ul>
            {foreach $prop->getAllowedValuesObjects() as $key => $item_value}
                <li class="filter-clickable_item {if isset($filters_allowed_sorted[$prop.id][$key]) && ($filters_allowed_sorted[$prop.id][$key] == false)}disabled-property{/if}">
                    ...
                    <label for="cb_{$prop.id}_{$item_value@iteration}" title="{$item_value.value}" style="background-color:{$item_value.color}">
                        {if $item_value.image}
                            <img src="{$item_value.__image->getUrl(34, 34, 'axy')}">
                        {/if}
                    </label>
                </li>
            {/foreach}
