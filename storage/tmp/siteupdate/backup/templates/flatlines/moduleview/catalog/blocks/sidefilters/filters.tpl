...
<div class="sidebar sec-filter rs-filter-section{if $smarty.cookies.filter} expand{/if}" data-query-value="{$url->get('query', $smarty.const.TYPE_STRING)}">
    ...
    <form method="GET" class="filters rs-filters" action="{urlmake filters=null pf=null bfilter=null p=null}" autocomplete="off">
        ...
        <div class="sidebar_menu_buttons">
            ...
            <a href="{urlmake pf=null p=null bfilter=null filters=null escape=true}" class="theme-btn_reset rs-clean-filter{if empty($filters) && empty($basefilters)} hidden{/if}">{t}Сбросить фильтр{/t}</a>
        </div>
    </form>
</div>