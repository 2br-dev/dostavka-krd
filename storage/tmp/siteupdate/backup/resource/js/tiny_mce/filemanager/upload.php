...
if (isset($_POST['path']))
{
   $storeFolder = $base_path.$_POST['path'];
   $storeFolderThumb = $_POST['path_thumb'];
}
else
{
   $storeFolder = $base_path.$current_path.$_POST["fldr"]; // correct for when IE is in Compatibility mode
   $storeFolderThumb = $thumbs_base_path.$_POST["fldr"];
}

$path_pos  = strpos($storeFolder, $base_path.$current_path);
...
{
	$i++;
	if ($path == $base_path.$current_path) $cycle = FALSE;
...
if ( ! empty($_FILES))
{
	...
	if (in_array(fix_strtolower($info['extension']), $ext) && $allow_to_upload)
	{
		...
		$targetPathThumb = $base_path.$storeFolderThumb;
		...
		if ($is_img)
		{
			...
			{
				if( ! new_thumbnails_creation($targetPath,$targetFile,$_FILES['file']['name'],$base_path.$current_path,$relative_image_creation,$relative_path_from_current_pos,$relative_image_creation_name_to_prepend,$relative_image_creation_name_to_append,$relative_image_creation_width,$relative_image_creation_height,$relative_image_creation_option,$fixed_image_creation,$fixed_path_from_filemanager,$fixed_image_creation_name_to_prepend,$fixed_image_creation_to_append,$fixed_image_creation_width,$fixed_image_creation_height,$fixed_image_creation_option))
				{
					$memory_error = FALSE;
				}
				else
            ...
            if ($config->watermark && in_array(fix_strtolower($info['extension']), array('jpg', 'gif', 'png'))) {
                $file = new \RS\Img\File($targetFile);
                $img = new \RS\Img\Core(\Setup::$ROOT, '', '');
                $img->putWatermark($file);
            }
