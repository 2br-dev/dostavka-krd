...
*/
abstract class AbstractType extends \RS\Html\AbstractHtml
{
    public
        $wrap_var,
    ...
    protected
        ...
        $title,
        $value = '',
        $prefilter_list = array();
    
    function __construct($key, $title, $options = array())
    {
        $this->key = $key;
        $this->title = $title;
        $this->type_array = array(1 => t('меньше, чем'), '2' => t('больше, чем'), 3 => t('равно'));
    ...
    */
    function getKeyVal()
    {
        if ($this->value=='' && $this->emptynull) return array();
        return array($this->key => $this->getValue());
    }
    ...
    function modificateQuery(\RS\Orm\Request $q)
    {
        if ($this->modificate_query_callback) {
            ...
            if (!($q instanceof \RS\Orm\Request)) {
                throw new \RS\Exception(t('Callback функция должна возвращать объект \RS\Orm\Request'));
            }
        }
    ...
    */
    ...
    {
        return \RS\Db\Adapter::escape($str);
    }
    ...
    public function getParts($current_filter_values, $exclude_keys = array())
    {
        ...
        if ($this->getNonEmptyValue() !== null) {
            ...
            $parts[] = array(
                ...
                'href_clean' => \RS\Http\Request::commonInstance()->replaceKey(array($this->wrap_var => $without_this) + $exclude) //Url, для очистки данной части фильтра
            );
        }
        return $parts;
    }
