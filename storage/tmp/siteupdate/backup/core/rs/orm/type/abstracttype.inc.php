...
abstract class AbstractType
{
    ...
    protected $listfunc_param = array(); //Дополнительные параметры для $listfunc
    protected $tree_function;
    protected $tree_function_params = array();
     ...
     * @return static
     */
    public function setTree($tree_function)
    {
        $this->tree_function = $tree_function;
        $this->tree_function_params = array_slice(func_get_args(), 1);
        return $this;
    }
    ...
    {
        $result = $this->default;
        if ($db_format && !in_array(gettype($result), array('NULL', 'bool', 'string'))) {
