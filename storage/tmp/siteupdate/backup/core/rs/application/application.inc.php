...
class Application
{
    ...
    protected
        ...
        $js = array(
            self::POSITION_HEADER => array(),
            self::POSITION_FOOTER => array()
        ),
        $css = array(
            self::POSITION_HEADER => array(),
            self::POSITION_FOOTER => array()
    ...
    protected $debug_modes = array( //Массив видов режима отладки
    ...
    {
        return json_encode($this->js_vars, (defined('JSON_UNESCAPED_UNICODE')) ? JSON_UNESCAPED_UNICODE : 0);
    }
    ...
    function getJs($position = self::POSITION_HEADER, $absolute = false)
    {
        ...
        $result = EventManager::fire('getjs', array(
            'js_array' => $js_array
        ));
    ...
    {
        if (!isset($this->css[$position])) return array();
        ...
        $result = EventManager::fire('getcss', array(
            'css_array' => $css_array
        ));
