...
class Tools
{
    ...
    public static function dateExtend($format, $timestamp)
    {
        ...
        if (strpos($format, '%k') !==false) {
            ...
            elseif ($timestamp > (time() - 60*60*24*31)) 
            {
                ...
                $format = str_replace('%k', $days." ".\RS\Helper\Tools::verb($days, t("день"), t("дня"), t("дней")).t(" назад"), $format);
            }
            else 
            {
        ...
        if (strpos($format, '%dw') !== false) {
            $day_of_week = array(t('Вс'), t('Пн'), t('Вт'), t('Ср'), t('Чт'), t('Пт'), t('Сб'));
            $format = str_replace('%dw', $day_of_week[date('w', $timestamp)], $format);
        }

        if (strpos($format, '%dW') !== false) {
            $day_of_week = array(t('воскресенье'), t('понедельник'), t('вторник'), t('среда'), t('четверг'), t('пятница'), t('суббота'));
            $format = str_replace('%dW', $day_of_week[date('w', $timestamp)], $format);
        }
    ...
    */
    public static function compareVersion($need, $version, $compare = '>=')
    {
        ...
        throw new \RS\Exception(t('Передан некорректный тип сравнения версий'));
    }    
