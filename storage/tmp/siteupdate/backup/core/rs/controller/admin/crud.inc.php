...
abstract class Crud extends Front
{
    ...
    protected $user_post_data = array(); //Этот массив мержится с массивом POST
    /** @var EntityList */
    protected $api;
    /** @var TreeList */
    protected $tree_api;
    ...
    function actionTreeMultiEdit()
    {
        ...
        $this->multiedit_check_func = array($this->getTreeApi(), 'multiEditCheck');
    ...
    public function actionTreeAdd($primary_key_value = null)
    {
        ...
        if ($primary_key_value === null) {
            ...
            $helper->setTopTitle(t('Редактировать') . " {$this->tree_entity_type_accusative} {title}");
        }
        ...
        if ($this->url->isPost() && !empty($ids)) {
            ...
            foreach ($allow_keys as $key => $val) {
                if (isset($obj['__' . $key])) {
                    ...
                    if ($property instanceof OrmType\ArrayList && !isset($post[$key])) $post[$key] = array();
                }
            }
