{if !$url->isAjax()}
<div class="crud-ajax-group crud-view-table-category {if $smarty.cookies.viewas == 'table'}left-up{/if}">
            <div class="updatable" data-url="{urlmake}">
{/if}
                <div class="top-toolbar viewport">
                    <div class="c-head">
                        <h2 class="title">{$elements.formTitle} {if isset($elements.topHelp)}<a class="help-icon" data-toggle-class="open" data-target-closest=".top-toolbar">?</a>{/if}</h2>
                ...
                {if $elements.category}
                    <div class="collapsed viewport">
                        <div class="path">
                            <i class="zmdi zmdi-folder-outline icon" data-toggle-class="left-open" data-target-closest=".crud-view-table-category"></i>
                            {$elements.category->getPathView()}
                        </div>
                        <div class="buttons">
                            <a class="select-tree btn btn-default" data-toggle-class="left-open" data-target-closest=".crud-view-table-category">
                ...
                <div class="columns">
                    <div class="column left-column">
                        <div class="filter-back" data-filter-placement="left">
                            {if $elements->getCategoryFilter()}
                                {$elements->getCategoryFilter()->getView()}
                            {/if}
                        </div>

                        <div class="viewport">
                            {if $elements->getCategoryFilter()}
                                {$elements->getCategoryFilter()->getPartsHtml()}
                            {/if}
                        </div>

                        <form method="POST" enctype="multipart/form-data" action="{urlmake}" id="tree-form" class="twisted-left">
                            {if isset($elements.category)}
                                {$elements.category->getView()}
                            {/if}

                            {if $elements->getCategoryBottomToolbar()}
                            <div class="bottom-toolbar">
                                <div class="viewport">
                                    {$elements->getCategoryBottomToolbar()->getView()}
                                </div>
                            </div>
                            {/if}
                        </form>
                            ...
                            <div class="view-control">
                                ...
                                <div class="view-switcher">
                                    <a class="view-as-tree-table" data-remove-class="left-up"
                                                                  data-target-closest=".crud-view-table-category"
