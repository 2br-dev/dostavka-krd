...
use \RS\Cache\Manager as CacheManager,
    \RS\HashStore\Api as HashStoreApi;
...
class Api extends \RS\Module\AbstractModel\BaseModel
{
    ...
    */
    function prepareUpdateInfo( $product )
    {
        ...
        if ($response === false) {
            return $this->addError(t('Невозможно соедениться с сервером обновлений'));
        } else {
            if ($response['success'] != true) {
                return $this->addError($response['error']);
            }
    ...
    * @return array
    */
    function compareVersions($skipped_modules = array())
    {
        $result = array();
        ...
        foreach($this->data['serverVersions'] as $module => $data) {
            ...
            if ($module == '@core') {
                $item['checkbox_attr'] = array('checked' => 'checked', 'disabled' => 'disabled', 'class' => 'always-checked');
            }
        ...
        foreach($modules as $module) {
            ...
            elseif ($module{0} == '#') {
                ...
                $install_map[] = array(
                    ...
                    'callback_params' => array($template),
                    'status' => t("Обновление шаблона %0", array($template))
                );                
            }
            else {
                $install_map[] = array(
                    'callback' => 'updateModule',
                    'callback_params' => array($module),
                    'moduleName' => $update_data['serverVersions'][$module]['name'],
                    'status' => t("Обновление модуля %0", array($update_data['serverVersions'][$module]['name']))
                );
            }
        }
    ...
    */
    function doUpdate()
    {
        ...
        if ($pos < count($this->data['installMap'])) {
            ...
            $params = isset($this->data['installMap'][$pos]['callback_params']) ? $this->data['installMap'][$pos]['callback_params'] : array();
            if ( ($update_result = call_user_func_array(array($this, $method), $params)) !== true ) {
                
                if ($update_result == '@timelimit') { //Запрос на повторный вызов, для операций, которые выполняются больше max_execution_time
                    return $this->getNextStepInfo() + array('need_restart' => true);
                }
                ...
                if (isset($this->data['installMap'][$pos]['moduleName'])) {
                    $module = t("Модуль '%0'", array($this->data['installMap'][$pos]['moduleName']));
                } else {
                    $module = null;
                }
    ...
    */
    private function getUpdateFilename()
    {
        ...
        if ($response === false) {
            return t('Невозможно соедениться с сервером обновлений. Попробуйте повторить попытку позже');
        } else {
            if ($response['success'] != true) {
        ...
        $core_file_data = $response['size']>0 ? array(array(
            'name' => 'core',        
            'type' => 'core',
            'size' => $response['size'],
    ...
    */
    ...
    {
        $exclude_folders = array('modules', 'templates');
    ...
    */
    ...
    /**
    * Возвращает количество дней в течении которых доступно обновление скрипта
    * 
    * @return integer
    */
    ...
    */
    function checkUpdates()
    {
        ...
        if ($prepare === true) {
            $skipped_modules = HashStoreApi::get(self::SKIPPED_MODULE_STORE_KEY, array());
