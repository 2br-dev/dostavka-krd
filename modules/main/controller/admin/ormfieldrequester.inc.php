...
class OrmFieldRequester extends Front
{
     ...
     * @throws \SmartyException
     */
    protected function recursiveGetTreeBranchesHtml(AbstractTreeListIterator $tree, array $ids, $load_recursive = false, $id_field = null, array $result = array())
    {
        ...
        foreach ($tree as $node) {
            ...
            if (in_array($object[$id_field], $ids)) {
                $view = new ViewEngine();
                $view->assign(array(
                    'iterator' => $node->getChilds(),
                    'load_recursive' => $load_recursive,
                ));
                $result[$object[$id_field]] = $view->fetch('%system%/coreobject/type/form/treelistbox_branch.tpl');
            }
