...
class Api extends \RS\Module\AbstractModel\BaseModel
{
    ...
    */
    ...
    {
        $path_mask = \Setup::$PATH.\Setup::$MODULE_FOLDER.'/install'.\Setup::$MODULE_TPL_FOLDER.'/lang/%LANG%/license.htm';
    ...
    */
    public function checkDbConnect()
    {
        ...
        if ( \RS\Db\Adapter::connect() === false ) {
            return t('Не удается соедениться с базой данных. Проверьте параметры соединения.');
        }
    ...
    function getInstallSteps($modules, $config)
    {
        ...
        foreach($modules as $module) {
            ...
            if ($mod_config = $item->getConfig()) {
                ...
                $caption = t("Установка модуля '%0'", array($mod_config['name']));
                $steps[] = array('action' => 'doInstall', 'module' => $module, 'caption' => $caption);
            }
        }
        
        $steps[] = array('action' => 'doAllowDbWrite', 'caption' => t('Подготовка к установке демо данных'));
        ...
        if ($config['set_demo_data']) {
            foreach($module_items as $module => $mod_item) {
                ...
                if ($install && $install->canInsertDemoData()) {
                    $caption = t("Установка данных модуля '%0'", array($mod_item->getConfig()->name));
                    $steps[] = array('action' => 'doInsertDemoData', 'module' => $module, 'caption' => $caption);
                }
            }
        }
        
        $steps[] = array('action' => 'doAfterInstall', 'caption' => t('Конфигурирование модулей'));
    ...
    public function doAfterInstall()
    {
        ...
        foreach($status['stack'] as $module) {
            ...
            if ($install = $mod_item->getInstallInstance()) {
                ...
                if ($result !== true) {
                    return $this->makeError(t("Модуль '%0'", array($module)), $result);
                }
            }
        }
        return true;
    ...
    */
    ...
    {        
        ...
        if ($status['pos'] < count($status['steps'])) {
            ...
            $install_result = call_user_func_array(array($this, $do['action']), array_diff_key($do, array('action' => true, 'caption' => true)));
    ...
    */
    public function doInsertDemoData($module, $params = array())
    {
    ...
    */
    ...
    {        
        ...
        if ( ($install_result = $current_module->install()) !== true ) {
            ...
            return $this->makeError(t("Модуль '%0'", array($mod)), $install_result);
        }
        return true;
    }
    ...
    */
    protected function finishInstall()
    {
        ...
        $php_deny_folders = array('/cache', '/storage', '/templates');
    ...
    */
    public function getInstallStack()
    {
        ...
        $first_modules = array('menu', 'site', 'main', 'users', 'modcontrol');
        ...
        while(count($modules)) {
            ...
            if ($before == count($modules)) {
                return t("Следующие модули зависят от несуществующих модулей '%0'", array(implode(',', array_keys($modules))));
            }
        }
