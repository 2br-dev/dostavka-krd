...
class Handlers extends HandlerAbstract
{
    ...
    function init()
    {
        $this->bind('cart.change', null, null, 10);
        $this->bind('cart.change', null, 'applyFixedCouponsToCart', 5);
        $this->bind('getroute');
        $this->bind('getmenus');
        $this->bind('user.auth');
    ...
    public static function cartChange($params)
    {
        ...
        if ($cart->getMode() == CART::MODE_SESSION) {
            $cart->applyOldCostDiscount();
        }
        DiscountApi::applyCouponPercentDiscountsToCart($cart);
    }
     ...
     * @throws RSException
     */
    public static function applyFixedCouponsToCart($params)
    {
        /** @var Cart $cart */
        $cart = $params['cart'];
        DiscountApi::applyCouponFixedDiscountsToCart($cart);
    }
    ...
    public static function getRoute(array $routes)
    {
        ...
        $routes[] = new Route('shop-front-multioffers',
        ...
        //Online платежи
        $routes[] = new Route('shop-front-onlinepay', array('/onlinepay/{PaymentType}/{Act:(success|fail|result)}/', '/onlinepay/{Act}/'), null, t('Online платежи'));
        //Список регионов
