...
class YandexMoney extends AbstractType
{
    ...
    */
    function getPayUrl(Transaction $transaction)
    {
        ...
        $success = $this->makeRightAbsoluteUrl($router->getUrl('shop-front-onlinepay', array(
           'Act'         => 'success',
           'inv_id'      => $inv_id,
           'PaymentType' => $this->getShortName(),
        )));
        
        $fail = $this->makeRightAbsoluteUrl($router->getUrl('shop-front-onlinepay', array(
           'Act'         => 'fail',
           'inv_id'      => $inv_id,
           'PaymentType' => $this->getShortName(),
        )));
        ...
        if ($transaction['order_id']) {
            ...
            if ($cart) {
                ...
                foreach ($products as $product) {
                    ...
                    //ФФД 1.05-1.1
                    $item['payment_mode'] = 'full_prepayment';
                    $item['payment_subject'] = $product['cartitem']['payment_subject'];
