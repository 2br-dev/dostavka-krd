...
class Cart
{
    ...
    protected $select_expression = array();
    ...
    protected $items = array(); // Элементы корзины
    protected $order_items = array(); // Элементы оформленного заказа
    ...
    public static function preOrderCart(Orm\Order $order = null)
    {
        ...
        //Вызовем событие
        EventManager::fire('cart.preorder', array(
            'cart' => $cart,
        ));
        return $cart;
    }
    ...
    function updateOrderItems(array $items)
    {
            ...
            if (!isset($this->items[$key])) {
                ...
                if(isset($item['type']) && $item['type'] == self::TYPE_COUPON){
                    if(empty($item['entity_id']) && isset($item['code'])){
                        $coupon = Orm\Discount::loadByWhere(array('code' => $item['code']));
        ...
        EventManager::fire('cart.updateorderitems.after', array('cart' => $this));
        $order = $this->getOrder();
        if ($order['trigger_cart_change']) {
            ...
            if ($this->mode == self::MODE_ORDER) {
                ...
                foreach ($product_items as $item) {
                    ...
                    if ($cartitem instanceof CartItem) {
                        foreach ($cartitem->getConcomitants() as $uniq => $concomitant_item) {
                            $concomitant_items[$uniq]['cartitem'] = $concomitant_item;
                ...
                if ($concomitant_item_ids) {
                    ...
                    foreach ($concomitant_items as $uniq => $concomitant_item) {
                        $product = $products[$concomitant_item['cartitem']['entity_id']];
                        $concomitant_items[$uniq]['cartitem']->setProduct($product);
                        $concomitant_items[$uniq]['product'] = $product;
                    }
                }
        ...
        if (!$cache || $this->cache_products === null) {
            ...
            if (!empty($ids)) {
                ...
                foreach($cartitem_product as $key => $cartitem) {
                    ...
                    if ($this->mode == self::MODE_ORDER) {
                        /** @var Product $product */
                        $product = $this->cache_products[$key]['product'];
     ...
     * @throws RSException
     */
    function addProduct($product_id, $amount = 0, $offer = 0, $multioffers = array(), $concomitants = array(), $concomitants_amount = array(), $additional_uniq = null, $source = null, $manual = false)
    {
        $api = new CatalogApi();
        /** @var Product $product */
        $product = $api->getOneItem($product_id);
        if (!$product) {
            throw new RSException(t('Невозможно добавить товар в корзину. Товар с ID %0 не найден', array($product_id)));
        }

        //Приготовим мультикомплектации
        $multioffers = $this->prepareMultiOffersInfo($product_id, $multioffers);

        $eresult = EventManager::fire('cart.addproduct.before', array(
        ...
        if (!$product_uniq) {
            ...
            if ($this->mode != self::MODE_PREORDER) {
                if ($this->items[$product_uniq]->replace()) {
                    ...
                    OrmRequest::make()
                        ->update(new CartItem())
                        ->set(array('user_id' => $this->items[$product_uniq]['user_id']))
                        ->where(array(
        ...
        EventManager::fire('cart.addproduct.after', array(
            'product_id' => $product_id,
            'amount' => $amount,
            'offer' => $offer,
            'multioffers' => $multioffers,
     ...
     * @throws RSException
     */
    function update($products = array(), $coupon = null, $safe = true, $manual = false)
    {
        $eresult = EventManager::fire('cart.update.before', array(
            'products' => $products,
            'coupon' => $coupon,
            'cart' => $this,
            'safe' => $safe,
            ...
            if (isset($this->items[$uniq]) && $this->items[$uniq]['type'] == self::TYPE_PRODUCT) {
                ...
                if (isset($product['amount'])) {
                    if (!$manual || !$cart_item->getForbidChangeAmount()) {
                        ...
                        $cart_item['amount'] = ceil($amount / $step) * $step;
                    }
                }
                ...
                if (!$safe) { //Установка приватных параметров товара
                    ...
                    if (isset($product['discount'])) {
                        if ($product['discount']) {
                            $cart_item->setExtraParam('personal_discount', $product['discount']);
                        } else {
                            $cart_item->unsetExtraParam('personal_discount');
                        }
                    }
                ...
                if (isset($this->cache_products)) {
                    $this->cache_products[$uniq]['cartitem'] = $cart_item;
                }
        ...
        EventManager::fire('cart.update.after', array(
            'products' => $products,
            'coupon' => $coupon,
            'cart' => $this,
            'safe' => $safe,
    ...
    {
        $eresult = EventManager::fire('cart.removeitem.before', array(
            'cart' => $this,
            'uniq' => $uniq,
            'manual' => $manual,
        ));
        ...
        if (isset($this->items[$uniq])) {
            if (!$manual || !$this->items[$uniq]->getForbidRemove()) {
                ...
                EventManager::fire('cart.removeitem.after', array(
                    'cart' => $this,
                    'uniq' => $uniq,
                    'item' => $deleted_item,
                    'manual' => $manual,
    ...
    function getCartData($format = true, $use_currency = true)
    {
        ...
        $min_limit = $shop_config['basketminlimit'];
        if ($use_currency) $min_limit = CurrencyApi::applyCurrency( $min_limit );
        ...
        if (($this->getTotalWeight()) < $min_weight_limit) {
            $result['errors'][] = t('Минимальный суммарный вес товаров должен составлять %0', array($min_weight_limit." ".$weight_unit));
            $result['has_error'] = true;
        }
        ...
        $eresult = EventManager::fire('cart.getcartdata', array(
            'cart' => $this,
            'cart_result' => $result,
            'format' => $format,
        ...
        if ($this->mode == self::MODE_SESSION) {
            $_SESSION[self::CART_SESSION_VAR] = array(
                $options => array(
            ...
            foreach($this->getProductItems() as $key => $product) {
                ...
                if (!isset($address->city)){  //если доставка самовывоз, и свой адрес пользователь не вводил
                    if ($delivery->getTypeObject()->isMyselfDelivery()) {  // для расчета ставки налога использовать регион(Город) из доставки
                        ...
                        if ($delivery_address = AddressApi::getAddressByCityid($city_id)) {
                ...
                $event_result = EventManager::fire('cart.getcartdata.producttaxlist', array(
                    'taxes' => $taxes,
                    'product' => $product,
                    'cart' => $this,
                ));
    ...
    function makeOrderCart()
    {
        ...
        foreach($session_items as $uniq => $item) {
            ...
            if ($item['type'] == self::TYPE_PRODUCT) {
                ...
                if ($personal_discount = $item->getExtraParam('personal_discount')) {
                    $new_item->setExtraParam('personal_discount', $personal_discount);
                }
                $new_item->setExtraParam('tax_ids', TaxApi::getProductTaxIds($products[$uniq]['product']));
    ...
    function getOrderData($format = true, $use_currency = true, $write_currency_liter = true)
    {
        ...
        $eresult = EventManager::fire('cart.getorderdata', array(
            'cart' => $this,
            'cart_result' => $result,
            'format' => $format,
            'use_currency' => $use_currency,
    ...
    function getTotalWeight($weight_unit = null)
    {
        ...
        foreach($in_basket as $n => $item) {
            ...
            $total_weight += $product->getWeight($item['cartitem']['offer'], $weight_unit) * ($amount > 0 ? $amount : 1); //Рассчитываем общий вес
        }
                ...
                foreach($product->getConcomitant() as $sub_product) {
                    foreach ($cart_item->getConcomitants() as $checked_concomitant) {
                        if ($sub_product['id'] == $checked_concomitant['entity_id']) {
                            $uniq = $this->addProduct($sub_product['id'], $checked_concomitant['amount'], 0, array(), array(), array(), null, $cart_source);
                            ...
                            if ($discounts = $checked_concomitant->getDiscounts()) {
                                $extra = array('discounts' => $discounts);
                                $this->update([$uniq => $extra], null, false);
                            }
                            break;
                        }
                    }
                    $cart_item->unsetExtraParam('concomitants'); // Очищаем сопутствующие у картитема после превращения их в картитемы
                }
            }
        }
    }
    ...
    protected function addProductData($result, $use_currency)
    {
        ...
        foreach($in_basket as $n => $item) {
            ...
            $result['items'][$n] = array(
                ...
                'single_weight' => $product->getWeight($item['cartitem']['offer']), //Вес единицы товара
                'discount' => 0,
                'sub_products' => array()
            );
            ...
            if ($module_config['check_cost_for_zero'] // Если нельзя покупать товары с 0 ценой
                && 0.0 === $cost
                && $cart_item->getExtraParam('source', '') === CartPage::CART_SOURCE_CART_PAGE // Проверяем, что товар добавлен "вручную"
            ) {
     ...
     * @return array
     */
    private function addFixedDiscountData($result, $discount, $max_percent = 100., $max_item_percent = 100., $discounted_items = array())
    {
                        ...
                        if (isset($data['sub_products']) && !empty($data['sub_products'])){
                            foreach ($data['sub_products'] as $sub_uniq=>&$subdata){
                                if ($subdata['checked'] && (!$discounted_items || isset($discounted_items['products'][$uniq]))){ //Если есть выбранные
                                    ...
                                    $subdata['total'] -= $one_item_discount;
                                    $subdata['total_base'] -= $one_item_discount;
                                    $subdata['total_discount'] += $one_item_discount;
                                }
                            }
                        }
                    }
     ...
     * @return array
     */
    private function correctFixedDiscount($result, $discount, $max_percent = 100., $max_item_percent = 100., $discounted_items = array())
    {
    ...
    protected function addOrderDiscountData($result, $use_currency)
    {
        ...
        $eresult = EventManager::fire('cart.before.addorderdata', array(
            'cart_result' => $result,
            'cart' => $this,
            'use_currency' => $use_currency
        ));
...
//            }
    ...
    protected function getCouponsUsage($coupons_items)
    {
        ...
        foreach($coupons_items as $key => $coupon_item) {
            ...
            foreach($products as $uniq => $product_item) {
                ...
                if ($coupon->isForAll()) {
                    ...
                    if (@in_array($product['id'], (array)$coupon_products['product']) || $in_group) {
                        $result[$key]['products'][$uniq] = $product['id'];
                    }
                    //Если есть сопутствующие, то и их добавим
                    if (!empty($concomitants)){
                        foreach ((array)$concomitants as $concomitant){
                            ...
                            if (@in_array($concomitant, (array)$coupon_products['product']) || $in_group) {
        ...
        $event_result = EventManager::fire('cart.getcouponsusage.after', array(
            'result' => $result,
    ...
    function addCoupon($code, array $extra = null)
    {
        ...
        $eresult = EventManager::fire('cart.addcoupon.before', array(
            'coupon' => $coupon
        ));
        if ($eresult->getEvent()->isStopped()) {
        ...
        EventManager::fire('cart.addcoupon.after', array(
            'coupon' => $coupon
        ));
        ...
        if ($order){ //Если заказ нашли, то получим его корзину и попрбуем добавить теже товары в текущую корзину
            ...
            foreach ($order_cart['items'] as $uniq_id => $item) {
                ...
                if ($product){ //Добавим товар в корзину
                    ...
                    $this->addProduct($product['id'], $cartitem['amount'], $cartitem['offer'], $mo_array, array(), array(), null, self::CART_SOURCE_REPEAT_ORDER);
                }
            }
            return true;
        }
     ...
     * @throws RSException
     */
    public function triggerChangeEvent()
    {
