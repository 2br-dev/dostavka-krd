    ...
    \Shop\Model\DeliveryType\InterfaceIonicMobile
{
    ...
    function getFormObject()
    {
        $properties = new \RS\Orm\PropertyIterator(array(
            ...
            'auto_create_order' => new Type\Integer(array(
            ...
            'timeout' => new Type\Integer(array(
                ...
                'hint' => t("Если в течении указанного времени от сервера СДЭК  не будет ответа, то процесс ожидания прервется"),
                'default' => 20,
            )),
        ) );
    ...
    private function requestPochtomat(\Shop\Model\Orm\Order $order, $tariff, \Shop\Model\Orm\Address $address = null)
    {
        ...
        if (empty($this->cache_pochtomates)) {
            ...
            $response = $this->api->getPvzList($params);
        ...
        if (isset($tariffs[$extra_ti])) {
            ...
            if ($tariff && in_array($tariff['regim_id'], array(2, 4))) { //Если нужны почтоматы
                if (isset($extra_info['code'])){ //Если есть место забора товара
                    ...
                    $order->addExtraInfoLine(t('Выбран пункт забора'), $extra_info['addressInfo'],null,'pvz',Order::EXTRAINFOLINE_TYPE_DELIVERY);
    ...
    private function requestDeliveryInfo(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null)
    {
        ...
        $request_array = array(
            'http' => array(
                ...
                'timeout' => $this->getOption('timeout') ?: 20
            )
        );
...
//            $query_info['tariffId'] = $tariffsList[0];  // id тарифа, не нужен, если есть tariffsList
        ...
        foreach ($items as $key=>$item){
            ...
            $length          = $property_length ? $property_length : $product->getDimensionsObject()->getLength();
            if ($length){
            ...
            $width          = $property_width ? $property_width : $product->getDimensionsObject()->getWidth();
            if ($width){
            ...
            $height          = $property_height ? $property_height : $product->getDimensionsObject();
            if ($height){
        ...
        if ( !isset(self::$cache_api_requests[$cache_key]) ) {
            ...
            $response = self::$cache_api_requests[$cache_key];
        }
        ...
        return self::$cache_api_requests[$cache_key] = $response;
    }
     ...
     */
    private function addErrorsToOrderAdminComment($action, $order, $errors)
    {
        if($action === NULL){
     ...
     * @throws \SmartyException
     */
    private function cdekGetPrintLabelForm(\Shop\Model\Orm\Order $order)
    {
        $extra_info = $order->getExtraInfo();
    ...
    function getDeliveryCost(\Shop\Model\Orm\Order $order, \Shop\Model\Orm\Address $address = null, \Shop\Model\Orm\Delivery $delivery, $use_currency = true)
    {
        ...
        if (isset($sxml['result'])){
            ...
            $cost = $this->applyExtraChangeDiscount($delivery, $cost, $order); //Добавим наценку или скидку
    ...
    private function getRightTaxForProduct(\Shop\Model\Orm\Order $order, \Catalog\Model\Orm\Product $product)
    {
        ...
        switch($tax_rate){
            ...
            case "18":
                ...
                break;
            case "20":
                $tax_id = 'delivery_recipient_vat_rate';
                break;
            case "0":
                $tax_id = 'VAT0';
                break;
        ...
        if (!isset($this->cache_city_id[$cache_key])) {
            ...
            if (!empty($file_name)) {
                ...
                while($cdek_city = fgetcsv($file, null, ';', '"')) { // Город найден при совпадении названия города и его области
                    ...
                    {
                        if(!empty($cdek_city) && mb_strtolower($city['title']) == mb_strtolower($cdek_city[$f['city_name']]) && stripos($region['title'], $cdek_city[$f['region_name']]) !== false ) {
            ...
            $this->writeToLog('Поиск в файле: '.$file_name.' Город из базы: '.$city['title'].' Регион из базы: '.$region['title'].' Найденый город по СДЕК: '.$cdekCity.' Найденый регион по СДЕК: '.$cdekRegion.' Найден КЛАДР по СДЕК:'.$cdekKladr.' Время на поиск '.$time.' Результат работы: ', $this->cache_city_id[$cache_key]);
        }
    ...
    /**
     * Получение городов доступных в СДЕК API
     * @return bool
     */
    public static function getCountries()
    {
        ...
        $countries = false;
        $data = Api::getInstance()->getLocationRegions(null); //json_decode(file_get_contents('http://integration.cdek.ru/v1/location/regions/json'));

        $f = fopen($file_path,'wa+');
        foreach ($data as $item){
            switch ($item->countryName){
                case "Turkey":
                    $item->countryName = "Турция";
                    break;
                case "China":
                    $item->countryName = "Китай";
                    break;
                case "Russia":
                    $item->countryName = "Россия";
                    break;
                case "United States":
                    $item->countryName = "США";
                    break;
                case "Uzbekistan":
                    $item->countryName = "Узбекистан";
                    break;
                case "Germany":
                    $item->countryName = "Германия";
                    break;
                case "Czech Republic":
                    $item->countryName = "Чехия";
                    break;
                case "Lithuania":
                    $item->countryName = "Литва";
                    break;
                case "United Arab Emirates":
                    $item->countryName = "ОАЭ";
                    break;
                case "Korea, South":
                    $item->countryName = "Южная Корея";
                    break;

            }
            if(!isset($countries[$item->countryCode]))
                fputcsv($f,array($item->countryCode,$item->countryName),';','"');
     ...
     * @param bool $flag
     */
    public static function createCountryFile($countryId, $data = array(), $flag = false)
    {
