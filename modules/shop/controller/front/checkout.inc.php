...
class Checkout extends Front
{
    ...
    public function actionIndex()
    {
        ...
        $this->app->redirect($this->router->getUrl('shop-front-checkout', array('Act' => 'address')));
    }
        ...
        if ($this->url->isPost()) { //POST
            ...
            if (!$logout && !$login) {
                ...
                if (in_array($this->order['user_type'], array('person', 'company')) && !$this->order['reg_autologin']) {
                    if (($pass_err = User::checkPassword($this->order['reg_openpass'])) !== true) {
                ...
                if ($this->order['user_type'] == 'noregister') {
                    ...
                    if ($config['require_phone_in_noregister'] && empty($this->order['user_phone'])) {
                ...
                if (!$this->order->hasError()) {
                    ...
                    EventManager::fire('statistic', array('type' => StatisticEvents::TYPE_SALES_FILL_ADDRESS));

                    $current_user = AppAuth::getCurrentUser();
                    $this->order['user_id'] = $current_user['id'];
                    $this->app->redirect($this->router->getUrl('shop-front-checkout', array('Act' => 'delivery')));
                }
            } //!logout && !login
        } else {
    ...
    public function actionDelivery()
    {
        ...
        if ($config['hide_delivery']) { //Если нужно проскочить шаг доставка
            ...
            EventManager::fire('statistic', $statistic_event);
    ...
    public function actionPayment()
    {
        ...
        if ($config['hide_payment']) { //Если нужно проскочить шаг оплата
            $this->app->redirect($this->router->getUrl('shop-front-checkout', array('Act' => 'confirm')));
        }
        ...
        if ($this->url->isPost()) {
            ...
            if ($this->order->checkData($sysdata, null, null, $work_fields)) {
                ...
                EventManager::fire('statistic', array('type' => StatisticEvents::TYPE_SALES_SELECT_PAYMENT_METHOD));
        ...
        if ($this->url->isPost()) {
            ...
            if (!$this->order->hasError() && $this->order->checkData($sysdata, null, null, $work_fields)) {
                ...
                if ($this->order->insert()) {
                    ...
                    EventManager::fire('statistic', array('type' => StatisticEvents::TYPE_SALES_CONFIRM_ORDER));

                    $this->order['expired'] = true; //заказ уже оформлен. больше нельзя возвращаться к шагам.
                    Cart::currentCart()->clean(); //Очищаем корзиу
                    $this->app->redirect($this->router->getUrl('shop-front-checkout', array('Act' => 'finish')));
                }
            }
        }
