...
class Api extends TreeCookieList
{
    ...
    {
        parent::__construct(new Menu(), array(
            'multisite' => true,
            'parentField' => 'parent',
            'nameField' => 'title',
            'aliasField' => 'alias',
    ...
    function moveElement($from, $to, $flag, OrmRequest $extra_expr = null, $new_parent_id = null)
    {
        ...
        if (!$extra_expr) {
            $extra_expr = OrmRequest::make()->where(array(
    ...
    function getAdminMenu($cache = true)
    {
        ...
        if ($cache) {
            ...
            return CacheManager::obj()->request(array($this, 'getAdminMenu'), false, $site_id, $user['id']);
        } else {
            $event_result = EventManager::fire('getmenus', array());
    ...
    function getMenuItems($root, $cache = true)
    {
        ...
        if ($cache) {
            ...
            return CacheManager::obj()->request(array($this, __FUNCTION__), $root, false, $site_id, $cache_id);
        } else {
            $root_orm = Menu::loadByWhere('(#id = "#root" or #alias = "#root") and site_id = #site_id', array(
                'id' => $this->id_field,
    ...
    public static function getMenuTypes($cache = true)
    {
        ...
        if (!isset($result) || !$cache) {
            $event_result = EventManager::fire('menu.gettypes', array());
            $result = array();
