...
class Profile extends AuthorizedFront
{
    ...
    function actionIndex()
    {
        ...
        if ($this->isMyPost()) {
            ...
            $theme = new ThemeItem($site_config['theme']);
            $version = @$theme->getThemeXml()->general->version;
            if (($theme->getName() == 'default' && HelperTools::compareVersion('3.0.0.42', $version)) ||
                ($theme->getName() != 'default' && HelperTools::compareVersion('1.0.0.16', $version)))
            {
                $this->url->checkCsrf();
            }
