...
class OfferBlock extends AdminBlock
{
    ...
    function actionIndex()
    {
        ...
        $main_offer = \Catalog\Model\Orm\Offer::loadByWhere(array(
        ...
        $this->offer_api->setFilter(array(
            'sortn:>' => 0,
            'product_id' => $this->product['id']
        ));
        
        $this->offer_api->applyFormFilter($filter, array(
            'product_id' => $this->product['id'],
        ...
        $url_pattern = $this->router->getAdminPattern(false, array(
            ':offer_page' => '%PAGE%', 
            'product_id' => $this->product['id'],
            'offer_page_size' => $page_size,
            'offer_filter' => $filter
    ...
    */
    function actionOfferEdit()
    {
        ...
        if ($this->url->isPost()) {
            ...
            $this->result->setSuccess( $this->offer_api->save($offer_id ?: null, array('id' => $offer_id) ) );
    ...
    function actionOfferMultiEdit()
    {
        ...
        if ($this->url->isPost() && !empty($ids)) {
            ...
            if (!empty($post)) {
                ...
                if ($offer->checkData($post, array(), array(), $doedit)) {
