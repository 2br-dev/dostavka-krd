...
class Api extends EntityList
{
    ...
    protected $work_list;
    protected $work_list_ids;

    function __construct()
    {
        parent::__construct(new Product(), array(
            'multisite' => true,
            'aliasField' => 'alias',
            'nameField' => 'title',
    ...
    public function addProductsPhotos($products)
    {
        ...
        foreach ($products as $product) {
            if (isset($product_photos[$product[$this->id_field]])) {
                ...
                $product['images'] = array(new PhotoStub());
            }
        }
        return $products;
    }
    ...
    */
    ...
    public function setSortOrder($field, $direction)
    {
        ...
        if (ConfigLoader::byModule($this)->list_order_instok_first
            && $field != $num_field
            && $num_field != 'dynamic_num') {
    ...
    public function addProductsDynamicNum($products, $auto_init_affiliate = true)
    {
        ...
        $dynamic_nums = Request::make()
            ->select('product_id, SUM(stock) as dynamic_num')
            ->from(new Xstock())
    ...
    public function addProductsMultiOffersInfo($products)
    {
        ...
        $use_multioffers = Request::make()
            ->select('DISTINCT product_id')
            ->from(new Orm\MultiOfferLevel())
            ->whereIn('product_id', $products_id)
            ->exec()->fetchSelected('product_id', 'product_id');
        
        $use_offers = Request::make()
    ...
    private function multiUpdateProductOffers( $ids, $excost, $all_prices, $update_price_round = false, $update_price_round_value = 0 )
    {
        ...
        $pricedata_offers = (array)Request::make()
            ->from(new \Catalog\Model\Orm\Offer())
            ->whereIn('product_id',$ids)
            ->where('sortn > 0')
            ->exec()
    ...
    function multiUpdateProductsAndOffersPrices( $ids = array(), array $excost )
    {
        ...
        foreach ($excost as $cost_id => $cdata) {
           ...
           if (isset($cdata['edit_multi']) && trim($cdata['plus_value']) !== "") {
             ...
             $all_prices[$cost_id] = Request::make()
                ->from(new Xcost())
    ...
    {  
        ...
        if (isset($data['recommended_arr']) && isset($data['recommended_arr']['product'])){
           $recomended =  $data['recommended_arr']['product'] == array(0=>'0') ? serialize(array()) : serialize($data['recommended_arr']);
           Request::make()
        ...
        if (isset($data['concomitant_arr']) && isset($data['concomitant_arr']['product'])){
            $concomitant = $data['concomitant_arr']['product'] == array(0=>'0') ? serialize(array()) : serialize($data['concomitant_arr']);
            Request::make()
                    ->from(new \Catalog\Model\Orm\Product())
                    ->set(array(
    ...
    */
    ...
    function getSameByCostProducts($product, $delta, $page_size)
    {
        ...
        if(ConfigLoader::byModule('catalog')->hide_unobtainable_goods == 'Y'){
    ...
    */
    ...
    {
        $in_products = Request::make()
            ->from(new \Catalog\Model\Orm\Product())
            ->where(array('barcode' => $barcode))
            ->object();
        $in_offers = Request::make()
            ->from(new \Catalog\Model\Orm\Offer())
            ->where(array('barcode' => $barcode))
            ->object();
                    ...
                    foreach ($url_filters as $property_dataparts){
                        ...
                        if ($property_id){ //id характеристики
                            foreach ($prop_data as $value_alias){
                                $props[$property_id][] = Request::make()
                                    ->from(new ItemValue())
                                    ->where(array(
                                        'site_id' => \RS\Site\Manager::getSiteId(),
                                        'prop_id' => $property_id
                                    ))
                                    ->where("(id = '#alias_or_id' OR alias = '#alias_or_id')", array('alias_or_id' => $value_alias))
                                    ->limit(1)
                                    ->orderby("alias != '#alias_or_id'", array('alias_or_id' => $value_alias))
                                    ->exec()
                                    ->getOneField('id', 0);
                            }
                        } else {
     ...
     * @throws RSException
     */
    private static function getEncodeDirFilterParamsArrayUrlParts($bfilters = array(), $pf = array(), $query = "")
    {
        $filters_as_url = array(); //Фильтры строкой
        ...
        if (!empty($pf)) { //Фильтр по бренду
            if ($config['use_seo_filters']) { //ЧПУ включен
                $seo_filter_types = array( //Список типов для адресов ЧПУ
                    Item::TYPE_LIST,
                    Item::TYPE_IMAGE,
                    Item::TYPE_COLOR,
     ...
     * @throws RSException
     */
    public static function encodeDirFilterParamsToUrl(\Catalog\Model\Orm\Dir $category = null, $bfilters = array(), $pf = array(), $query = "", $additional_params = array())
    {
        static
     ...
     * 
     * @param \Catalog\Model\Orm\Dir $category - объект категории
     * @param array $bfilters - базовые фильтры
     * @param array $pf - фильтры по характеристикам
     * @param string $query - строковый запрос
     * @return array
     * @throws RSException
     */
    public static function getCategoryPaginatorRouteParamsForProductsList(\Catalog\Model\Orm\Dir $category, 
                                                                            $bfilters = array(), 
                                                                            $pf = array(), 
                                                                            $query = "", $additional_params = array())
    {
    ...
    public function setWarehouseFilter($warehouses, $only_join_dynamic_num = false)
    {
        if ($warehouses) {
            $this->queryObj()
                ->select("SUM(XST.stock) as dynamic_num")
    ...
    public function getDataByBarcode($sku)
    {
        ...
        if ($product = $q->object()) { // если не найден товар
            ...
            $offer = $product['offers']['items'][0]->getValues(); // Берём первую комплектацию, т.к. был найден товар
            $offer_id = $offer['id'];
