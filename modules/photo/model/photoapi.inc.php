...
class PhotoApi extends \RS\Module\AbstractModel\EntityList
{
    ...
    protected
        ...
        */
        ...
        $uploadError = array(),
        $allow_mime = array('image/pjpeg', 'image/jpeg', 'image/png', 'image/gif', 'image/webp'),
    ...
    * @param bool $enable_resize -
    * 
    *  Для загрузки изображения с удалённого сервера: $copy = true, $enable_resize = false
    * 
    * @return bool|\Photo\Model\Orm\Image
    */
    function addFromUrl($url, $type, $linkid, $copy = false, $extra = null, $enable_resize = true, $prevent_duplication = false)
    {
        ...
        // если меньше 7.1 то webp не доступен
        if(!self::checkWebpRight($image_info['mime'])) {
            $this->uploadError[] = t("Поддержка формата WebP доступна при версии PHP не ниже 7.1");
            return false;
        }
        if (!isset($image_info[2]) || !in_array($image_info[2], array(IMAGETYPE_GIF,IMAGETYPE_JPEG,IMAGETYPE_PNG, IMAGETYPE_WEBP)))
        {
        ...
        if (  $result ) {
            ...
            foreach((array)$linkid as $one_linkid) {
                ...
                if ( $this->checkHashDuplicate($hash, $one_linkid, $type) && $prevent_duplication ) {
                    $this->uploadError[] = "{$file['name']}: ".t('Загружен дубликат файла для объекта %0', array($one_linkid));
                    $duplicate_counter++;
                    continue;
                }
    ...
    /**
     * Если найдено изображение с таким хэшем, то возвращает Image, иначе false
     * @param string $hash
     * @param int $linkid
     * @return bool(false)
     */
     ...
     * @return bool|\Photo\Model\Orm\Image
     */
    function uploadImage(array $file, $type = null, $linkid = null, $title = null, $extra = null, $enable_resize = true)
    {
        ...
        {
            // если меньше 7.1 то webp не доступен
            if(!self::checkWebpRight($file['type'])) {
                $this->uploadError[] = t("Поддержка формата WebP доступна при версии PHP не ниже 7.1 ".$file['type']);
                return false;
            }
            $image_info = getimagesize($file['tmp_name']);
            if (!in_array($file['type'], $this->allow_mime) || !isset($image_info[2])
                || (!in_array($image_info[2], array(IMAGETYPE_GIF,IMAGETYPE_JPEG,IMAGETYPE_PNG, IMAGETYPE_WEBP))))
            {
    ...
    */
    ...
    /**
     * Проверка на доступность поддержки webp
     *
     * @param $type - тип картинки image/webp
     * @return bool
     */
    static function checkWebpRight($type)
    {
        return ($type == 'image/webp' && (version_compare(phpversion(),'7.1.0') < 0)) ? false : true;
    }
