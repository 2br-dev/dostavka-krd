...
if (!file_exists($base_path.$current_path.$rfm_subfolder.$subdir))
{
    $subdir = '';
    if (!file_exists($base_path.$current_path.$rfm_subfolder.$subdir))
    {
...
while($cycle && $i < $max_cycles){
    ...
    if (file_exists($base_path.$current_path.$parent."config.php"))
    {
		require_once $base_path.$current_path.$parent."config.php";
		$cycle = FALSE;
    }
...
if (!is_dir($base_path.$thumbs_path.$subdir))
{
    create_folder(FALSE, $base_path.$thumbs_path.$subdir);
}
...
$files = scandir($base_path.$current_path.$rfm_subfolder.$subdir);
$n_files=count($files);
...
foreach($files as $k=>$file){
    ...
    elseif(is_dir($base_path.$current_path.$rfm_subfolder.$subdir.$file)){
	$date=filemtime($base_path.$current_path.$rfm_subfolder.$subdir. $file);
	if($show_folder_size){
		$size=foldersize($base_path.$current_path.$rfm_subfolder.$subdir. $file);
	} else {
		$size=0;
	}
	...
	$file_path=$base_path.$current_path.$rfm_subfolder.$subdir.$file;
...
<!-- header div end -->
	...
	<div class="span12">
	    <?php if(@opendir($base_path.$current_path.$rfm_subfolder.$subdir)===FALSE){ ?>
	    <br/>
		...
		foreach ($files as $file_array) {
		  $file=$file_array['file'];
			...
			if($file!='..' && $file!=$new_name){
			    //rename
			    rename_folder($base_path.$current_path.$subdir.$file,$new_name,$transliteration);
			    $file=$new_name;
			}
			//add in thumbs folder if not exist
			if (!file_exists($base_path.$thumbs_path.$subdir.$file)) create_folder(false,$base_path.$thumbs_path.$subdir.$file);
			$class_ext = 3;
			if($file=='..' && trim($subdir) != '' ){
		    $src = explode("/",$subdir);
			...
			$file=$file_array['file'];
			    if($file == '.' || $file == '..' || is_dir($base_path.$current_path.$rfm_subfolder.$subdir.$file) || in_array($file, $hidden_files) || !in_array(fix_strtolower($file_array['extension']), $ext) || ($filter!='' && $n_files>$file_number_limit_js && stripos($file,$filter)===false))
				    continue;
				...
				$file_path1=($base_path.$current_path.$rfm_subfolder.$subdir.$file1);
				if(file_exists($file_path1)){
				    $i = 1;
				    $info=pathinfo($file1);
				    while(file_exists($base_path.$current_path.$rfm_subfolder.$subdir.$info['filename'].".[".$i."].".$info['extension'])) {
					    $i++;
				    }
				...
				//add in thumbs folder if not exist
				if(!file_exists($base_path.$src_thumb)){
			    try {
			    	if(!create_img($file_path, $base_path.$src_thumb, 122, 91)){
							$src_thumb=$mini_src="";
						}else{
							new_thumbnails_creation($base_path.$current_path.$rfm_subfolder.$subdir,$file_path,$file,$base_path.$current_path,'','','','','','','',$fixed_image_creation,$fixed_path_from_filemanager,$fixed_image_creation_name_to_prepend,$fixed_image_creation_to_append,$fixed_image_creation_width,$fixed_image_creation_height,$fixed_image_creation_option);
						}
			    } catch (Exception $e) {
						$src_thumb=$mini_src="";
			    }
